...
-- ================================================================
-- 10. request_idempotency：请求层幂等（X-Request-ID 防重复提交）
-- ================================================================
CREATE TABLE IF NOT EXISTS request_idempotency (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL COMMENT '请求方用户',
    request_id VARCHAR(64) NOT NULL COMMENT '客户端 X-Request-ID，重试时复用同一值',
    endpoint VARCHAR(128) NOT NULL DEFAULT '' COMMENT '端点标识',
    status VARCHAR(32) NOT NULL DEFAULT 'processing' COMMENT 'processing|completed|failed',
    response_status INT NULL COMMENT '首次执行的 HTTP 状态（重复请求缓存用）',
    response_body TEXT NOT NULL COMMENT '响应快照（超过 16KB 不缓存）',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_request_idempotency (user_id, request_id),
    INDEX idx_request_idempotency_user (user_id),
    INDEX idx_request_idempotency_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='请求层幂等（客户端重复提交去重）';
