...
def test_build_session_keys_uses_consistent_suffixes() -> None:
    keys = stm_memory.build_session_keys("stm", "tenant-1", "user-1", "session-1")
...
def test_decode_model_accepts_bytes_and_ignores_non_dict_payload() -> None:
    ...
    decoded = stm_memory.decode_model(meta.model_dump_json().encode("utf-8"), SessionMeta)

    assert decoded == meta
    assert stm_memory.decode_model('["not", "a", "dict"]', SessionMeta) is None
    assert stm_memory.decode_model(None, SessionMeta) is None
...
def test_decode_messages_skips_invalid_payload_and_restores_time_order() -> None:
    ...
    decoded = stm_memory.decode_messages(
        [
...
def test_split_messages_for_compression_keeps_recent_rounds() -> None:
    ...
    messages_to_compress, messages_to_keep = stm_memory.split_messages_for_compression(
        messages,
...
def test_extract_summary_from_response_reads_json_block() -> None:
    summary = stm_memory.extract_summary_from_response(
...
def test_message_score_uses_existing_millisecond_timestamp() -> None:
    ...
    assert stm_memory.message_score(message) == 1_700_000_000_123
...
def test_message_score_falls_back_to_current_time_for_second_timestamp(
    ...
    assert stm_memory.message_score(message) == 123456
