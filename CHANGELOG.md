...
- 事件 payload 顶层固化 `tenant_id`，worker 消费后恢复 TenantContext；
  索引任务回写路径显式携带租户。
- **请求层幂等**：新增 `request_idempotency` 表与 `X-Request-ID` 去重——
  前端在"发送"动作生成一次 request_id、网络重试复用同一值，服务端以
  `(user_id, request_id)` 唯一键返回首次结果（SSE 流不可重放，重复 409）。
  接入 `POST /conversations`、`POST /upload`、`POST /langgraph/query`。
  与事件层幂等（processed_events / turn_id）分工：请求层防重复提交，
  事件层防投递重放。

**文档**: 新增 [docs/modules/08-SaaS多租户架构.md](docs/modules/08-SaaS多租户架构.md)，
README / API / 配置与数据字段全览同步更新。

**表变更**：新表 `tenants` / `tenant_memberships` / `request_idempotency`；
9 张业务表新增 `tenant_id` 列并重建唯一键。
