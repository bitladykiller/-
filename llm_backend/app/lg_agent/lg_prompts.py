...
v3.17: 提示模板外置为 lg_prompts.yaml。优先从 YAML 加载（支持热更新），
       加载失败时降级为下方硬编码默认值（向后兼容）。
"""
...
# ================================================================== #
# 默认 Prompt（硬编码降级值）— 当 YAML 文件加载失败时使用
# ================================================================== #
...
# ================================================================== #
# YAML 加载器 — 优先从 lg_prompts.yaml 读取，失败时降级为默认值
# ================================================================== #

def _load_prompts_from_yaml() -> dict[str, str]:
    """从 lg_prompts.yaml 加载 Prompt 模板。

    加载策略：
    1. 尝试从与当前文件同目录的 lg_prompts.yaml 读取
    2. 使用 YAML 安全加载器解析
    3. 缺失字段自动降级为 _DEFAULT_* 值
    4. YAML 解析失败时全部降级（记录 warning 日志）

    Returns:
        {key: prompt_text} 映射，key 为 YAML 字段名（如 "router_system"）。
    """
    import yaml

    yaml_path = Path(__file__).parent / "lg_prompts.yaml"
...
# 模块级加载（import 时执行一次）
_yaml_prompts = _load_prompts_from_yaml()


def _get_prompt(yaml_key: str, default: str) -> str:
    """获取 Prompt：优先 YAML，缺失时降级为默认值。"""
    return _yaml_prompts.get(yaml_key, default)
...
ROUTER_SYSTEM_PROMPT = _get_prompt("router_system", _DEFAULT_ROUTER_SYSTEM)
RETRIEVAL_PLAN_ROUTER_PROMPT = _get_prompt("retrieval_plan_router", _DEFAULT_RETRIEVAL_PLAN_ROUTER)
GENERAL_QUERY_SYSTEM_PROMPT = _get_prompt("general_query", _DEFAULT_GENERAL_QUERY)
GUARDRAILS_SYSTEM_PROMPT = _get_prompt("guardrails", _DEFAULT_GUARDRAILS)
REACT_SYSTEM_PROMPT = _get_prompt("react_system", _DEFAULT_REACT_SYSTEM)
REACT_ANSWER_CHECK_PROMPT = _get_prompt("react_answer_check", _DEFAULT_REACT_ANSWER_CHECK)
