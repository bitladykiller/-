...
```text
1. Redis: get_summary + get_recent_messages
   失败 → 降级空摘要/空列表（warn once）
2. 若 user_id 可解析为 >0:
     profile_reader(uid, redis)
     失败 → {}
3. 若 ltm_enabled:
     milvus_ltm.hybrid_search(tenant, user, user_input)
     失败 → []
4. 返回 AgentMemoryState
```
...
2. append user + assistant MessageRecord
...
5. 可选：刷新 LTM 命中统计 update_memory_hit_info
```

所有 Redis/Milvus 异常以**降级 + 单次警告**为主，避免刷屏和拖垮主对话。
...
| 读取 | 失败降级 |
|---|---|
| `get_summary` + `get_recent_messages` | summary=None，messages=[] |
| `profile_reader(uid)`（uid 可转 int） | `{}` |
| `milvus_ltm.hybrid_search(...)`（ltm_enabled） | `[]` |

同类 Redis 错误用 `_warn_once` 避免刷屏。

### `MemoryMiddleware.after_agent`（职责摘要）

写入本轮 user/assistant → `should_compress` 则压缩 → 抽取 LTM / 更新画像 → 可选 `update_memory_hit_info`。  
实现细节见上文 §3 / §14–15。
