...
| `graph/timing.py` | `timed_node` 节点计时包装 + request_id 关联（见 §7.5） |
...
| `retrievers/rag_query_formalize.py` | RAG 查询书面化改写（`formalize_rag_query`，超时/失败逐级回退） |
...
| `kg/tenant_cypher.py` | Cypher 执行闸口的确定性租户约束注入（`WITH $__tenant_id` 改写，不依赖 LLM 自觉） |
...
- 表：`conversations`  
- 只存**元信息**（标题、状态、类型、时间、user_id、tenant_id）  
- `tenant_id` 是组织边界列（VARCHAR(64)，默认 `"default"`）；索引：
  `idx_conversation_tenant_user(tenant_id, user_id)`、`idx_conversation_tenant_id(tenant_id, id)`  
- **不存**逐条消息  
...
| 方法 | 作用 |
|---|---|
| `create(tenant_id, user_id)` | 在指定租户下插入会话，返回 id |
| `list_by_user(tenant_id, user_id)` | 租户内列表摘要（排除默认标题） |
| `get_owned(tenant_id, conversation_id, user_id)` | **租户∧id∧user 三条件**归属校验；不存在/非本人 → `ResourceNotFoundError` |
| `delete(tenant_id, conversation_id, user_id)` | **仅 MySQL**：先归属校验，再删会话元信息 + 兼容 `messages` 表 |
| `rename(tenant_id, conversation_id, user_id, name)` | 先归属校验，再改标题 |
...
| 方法 | 签名 | 返回 / 行为 |
|---|---|---|
| `create_conversation` | `(tenant_id: str, user_id: int) -> int` | 新会话 id |
| `get_user_conversations` | `(tenant_id: str, user_id: int) -> list[ConversationSummary]` | id/title/created_at/status/dialogue_type |
| `delete_conversation` | `(tenant_id: str, conversation_id: int, user_id: int) -> None` | 见下 |
| `update_conversation_name` | `(tenant_id: str, conversation_id: int, user_id: int, name: str) -> None` | 改标题 |
| `ensure_conversation` | `(tenant_id: str, user_id: int, conversation_id: int\|None) -> int` | 传 id→归属校验；None→自动创建（会话 id 唯一来源，杜绝 uuid 孤儿线程） |
| `list_messages` | `(tenant_id: str, conversation_id: int, user_id: int) -> list[dict[str, str]]` | 先归属校验，再读 MySQL messages |

**`delete_conversation` 步骤：**

1. `Repository.delete(tenant_id, conversation_id, user_id)`：MySQL 会话 + 兼容 `messages`  
2. `_clear_conversation_memories(*, tenant_id=..., user_id=str(...), session_id=str(id))`  
   - `redis_stm.clear_session(tenant_id, user_id, session_id)`  
   - `milvus_ltm.soft_delete_session_memories(tenant_id, user_id, session_id)`  
   - 失败 **只日志，不回滚 MySQL**  
...
| `delete`/`rename` | 不存在/非本人 → `ResourceNotFoundError`；经 API 映射 **HTTP 404**（统一文案防枚举） |
...
📌 **签名权威版在 附录 G.1**，这里只记关键点：

- `stream_agent_query(*, query, user_id, tenant_id, thread_id)`（keyword-only，以 tenant_id 开头）
- configurable 三元组：`{thread_id, user_id, tenant_id}`——thread_id≡STM/LTM session、
  user_id≡画像/记忆作用域、tenant_id≡租户隔离（SaaS 数据边界）
...
`stream_agent_query` 的签名与 astream 细节见 **附录 G.1**（唯一权威版，本节不重复）。
...
状态机全貌见 **§0 总览 mermaid 图** 与 **附录 A1 背诵链路**（两处与源码一致，
本节不重复粘贴）。关键结构一句话：`START → route_and_plan_query`（经
`routing_decision_edge` 分流）→ general 直达 `respond_to_general_query`；
rag_doc-query 经 `guardrails_node`（end→拒答进 after_response / continue→
按 `resolved_plan` 进五类 `execute_*`）→ 全部汇入 `after_response → END`。
...
完整温度表（含每个角色的 WHY）见 **附录 A4**（权威版，两处逐字一致），
本节不重复粘贴。
...
双限制的**权威数字**见 附录 A2.5（`max_attempts` / `recursion_limit` /
`transcript_window`），本节不重复粘贴。
...
```text
POST /api/langgraph/query (Form: query, conversation_id?)
  → thread_id = str(conversation_id)（不传则由服务端 create_conversation 创建；无 uuid4 兜底）
  → stream_agent_query → graph.astream(stream_mode="messages", configurable={thread_id,user_id,tenant_id})
...
| 函数 | 文件 | 入参 | 出参 | 失败 |
|---|---|---|---|---|
| `stream_agent_query` | agent_query_service | query, user_id, tenant_id, thread_id | astream 迭代器 | 异常上抛到 API（权威签名见 G.1） |
| `ConversationService.delete_conversation` | conversation_service | tenant_id, conversation_id, user_id | None | 归属失败 ResourceNotFoundError→API 404；记忆清理不抛 |
...
> 签名与源码逐字一致（v3.37）。📌=热路径 🔒=并发语义 ⚠️=历史踩坑。

### G.1 `application/agent_query_service.py`

#### 📌 `def stream_agent_query(*, query, user_id: int | str, tenant_id: str, thread_id: str) -> GraphStream`

组 `InputState(messages=[HumanMessage(query)])` →
`graph.astream(stream_mode="messages", config={configurable:
{thread_id, user_id: str(user_id), tenant_id}})`。configurable 是记忆与检索的
作用域约定：thread_id≡STM/LTM session、user_id≡画像/分域、tenant_id≡租户隔离
（SaaS 数据边界）。非 async 函数——返回异步迭代器本身。
...
- `ConversationService` 方法：`create_conversation(tenant_id, user_id)` /
  `get_user_conversations(tenant_id, user_id)`（过滤默认标题）/
  `delete_conversation(tenant_id, cid, uid)`（repo 删→`_clear_conversation_memories`：
  STM clear_session + LTM soft_delete_session_memories，**失败只记日志
  不回滚**——避免"库删了但接口 500"）/
  📌 `ensure_conversation(tenant_id, uid, cid|None) -> int`（传 id→归属校验；None→
  创建；⚠️ 会话 id 三位一体的唯一来源，杜绝 uuid 孤儿线程）/
  `list_messages(tenant_id, cid, uid)`（先归属后读 MessageRepository）/
  `update_conversation_name(tenant_id, cid, uid, name)`
- 模块私有 `_create_conversation/_get_user_conversations/_delete_conversation/
  ...
  repo 适配函数（db 会话第一参；业务参数均以 tenant_id 开头）

### G.3 `infrastructure/repository/`

**conversation_repository.py**：`create(tenant_id, user_id)`（默认标题「新会话」）/
`get_by_id(tenant_id, cid)` / `list_by_user(tenant_id, uid)`（排除默认标题+时间倒序）/
`get_owned(tenant_id, cid, uid)`（**租户∧id∧user 三条件**）/
`delete(tenant_id, cid, uid)`（⚠️ 归属校验→兼容清理历史 messages 表（表缺失
rollback 后重载对象）→删行；非本人 ResourceNotFoundError 统一文案防
枚举）/ `rename(tenant_id, cid, uid, name)`。

**message_repository.py**：`add_turn(tenant_id, cid, user_content,
assistant_content, *, turn_event_id=None)`（两行一次提交，append-only；
JOIN conversations 强制租户边界）/ `list_by_conversation(tenant_id, cid)`
（JOIN conversations 强制租户，id 正序→`{role, content, created_at(iso)}`）。
写入方：turn_completed 消费者。
