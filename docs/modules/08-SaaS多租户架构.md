...
## 8. 请求层幂等 + 异步事件：tenant_id 固化进 payload

### 8.0 请求层幂等（X-Request-ID，防客户端重复提交）

事件层幂等（§8.2/8.3）只能防"同一事件的投递重放"，**防不了"客户端把
同一操作发了两次"**——前端网络抖动自动重试时，每次请求都会生成新的
turn_id，服务端无法区分"重试"和"真实新请求"。补齐办法是请求层幂等：

```text
前端"用户点击"时生成一次 X-Request-ID（重试时复用同一值，不是重新生成）
        ↓
服务端以 (user_id, request_id) 唯一键去重（request_idempotency 表）
        ↓
首次：执行并记录 completed + 响应快照
重复且 completed：返回缓存响应（SSE 流不可重放 → 409）
重复且 processing/failed：409，客户端应换新 request_id
```

- 幂等键范围 `(user_id, request_id)`：request_id 由客户端生成（UUID），
  用户维度防跨用户碰撞
- 响应快照上限 16KB，超限只记状态（重复请求 409）
- 接入端点：`POST /api/conversations`、`POST /api/upload`（缓存响应）、
  `POST /api/langgraph/query`（SSE，流结束才标记 completed）
- 无 `X-Request-ID` 的请求放行（兼容老客户端）

两层幂等分工：

| 层 | 防什么 | 载体 |
|---|---|---|
| 请求层（request_idempotency） | 同一操作的重复提交（前端重试/连点） | `X-Request-ID` 头 |
| 事件层（processed_events） | 同一事件的投递重放（Stream 至少一次/崩溃续跑） | `turn_id`/`task_id` 固化进 payload |
...
| 请求层幂等（X-Request-ID） | ✅ | `(user_id, request_id)` 去重，防客户端重复提交 |
