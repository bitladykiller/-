...
```mermaid
flowchart LR
    FILE[MD/PDF/DOCX] --> PARSE[doc_parser.pipeline]
    PARSE --> CH[chunks]
    CH --> IDX[HybridSearcher.index]
    IDX --> MV[(Milvus)]
...
app/api/upload.py
app/shared/background_tasks.py
...
| `api/upload.py` | HTTP 上传与任务提交（见 03） |
| `shared/background_tasks.py` | 异步 `process_file` 状态机 |
...
> 业务触发：`IndexingService.process_file`（上传任务队列）  
> 下面按源码逐步展开，不是目录地图。
...
```text
POST /api/upload  (Form: file, user_id, mode=create|replace, doc_id?)
  validate 扩展名/MIME
  读入 + 大小限制 + 魔数 + content_hash=sha256
  落盘 uploads/{user_uuid}/{time}/...
  DocumentService.prepare_create | prepare_replace
    · replace 且 content_hash 与 MySQL 一致 → 返回 unchanged，不 submit
  task_manager.submit(run_document_indexing_job, file_info)
    → Redis task:pending
    → 后台 run_document_indexing_job:
         IndexingService.process_file
           校验 path/ext/mode/doc_id
           parse_document(path, doc_id=稳定 id)     ← 本文 1–6 节
           create → HybridSearcher.index(chunks, version=1)
           replace → HybridSearcher.reindex(doc_id)  # 软删+新 version
...
GET /api/documents/user/{user_id} 列表绑定 doc_id 与展示名
```

之后用户提问 → RetrievalPlan 可能选 `RAG_ONLY` / `PARALLEL` / `REACT`  
...
见 [02-API接口.md](02-API接口.md) 上传流程；任务状态流转直接看源码 `app/shared/background_tasks.py`。
...
检索计划为**能力标签**（`need_graph` / `need_rag` / `mode` / `complexity`），
由 `resolve_execution_plan` 得到 `resolved_plan` 后再进执行节点：

```mermaid
flowchart LR
    LLM[RetrievalPlanOutput<br/>能力标签] --> R[resolve_execution_plan]
    R --> RP[resolved_plan]
...
详见 [03 §5.5](03-对话与Agent主图.md) 与附录 A；主图状态机见 [00 §6–8](00-全流程图集.md)。
...
**检索是全局共享的，管理是按用户隔离的** —— 这是有意的设计，不是遗漏：

- Milvus chunk **没有 user 维度**，任何用户的提问都检索全部未软删文档
  （客服知识库定位：产品手册/政策对所有用户生效）
- MySQL `user_documents.user_id` 只决定**谁能替换/删除**这份文档
  （上传管理归属），不影响检索可见性
- 推论：上传即对全体用户生效。传错的文档必须尽快
  `DELETE /api/documents/user/{user_id}/{doc_id}` 撤下（软删后检索立即排除）

> **v3.35.0 起私有文档域已实现（特性开关，默认关）**：
> chunk schema 已含 `owner_id`；上传可传 `visibility=global|private`；
> `app_config.rag_visibility.enabled=true` 时检索按
> `owner_id in ("global", 当前用户)` 过滤（身份取自请求上下文）。
...
17. **task completed vs indexing success？** 队列状态 vs 业务 payload。  
...
- 入库：`DocumentService.prepare_create/replace` → `run_document_indexing_job` → 策略 2 软删+version → `apply_indexing_result`
...
`prepare_replace`：若库中 `content_hash` 与新文件 SHA256 均非空且相等 → 返回 `unchanged=true`，`upload` 不提交 task_queue / 不 reindex。
...
| `owner_scope_filter(user_owner, *, global_owner="global")` | 分域过滤：匿名→仅共享域；登录→`owner_id in [global, uid]`。⚠️ 存量 chunk 无该字段会被排除——开分域前必须全量 reindex |
...
| 📌 `insert_chunks(chunks, *, version=1, content_hash="", owner_id="global") -> int` | | 批量 embed→组 rows（含 owner_id 截 64）→insert；空入参 0 |
| `reindex_document(doc_id, chunks, *, content_hash="", owner_id="global")` | | 软删旧版→next_version→**强制对齐 chunk.doc_id**（防解析侧漂移）→insert |
| `hard_purge_soft_deleted(*, retention_seconds=7d, batch_limit=16384) -> int` | | 过期软删→按主键物理删；非法参数 0 |
| `_visibility_filter() -> str\|None` | | 分域开关关→None；开→从 **contextvars 取当前 user** 构造 owner_scope_filter（检索层零传参拿身份） |
...
| `index(chunks, *, version=1, content_hash="", owner_id="global") -> int` | 透传 insert_chunks |
| `reindex(doc_id, chunks, *, content_hash="", owner_id="global")` | 透传 reindex_document（replace 策略 2） |
| `soft_delete_document(doc_id)` / `hard_purge_soft_deleted(...)` | 透传 |
| 📌 `search(query, top_k=None, filter_expr=None)` | hybrid（候选取 max(final, rerank_top_k)）→ Reranker.available 则精排 → 截 final_top_k |
...
| 📌 `IndexingService.process_file(file_info) -> IndexingResult` | 路径/uid 宽松解析→存在性/类型校验→mode+doc_id→owner（默认取 rag_visibility.global_owner）→parse_document→空文档成功返回 chunks=0→replace: `searcher.reindex`（⚠️ 曾有 except TypeError 签名嗅探为测试替身让路，已删，契约以 ChunkIndexer Protocol 为准）/ create: `searcher.index(version=1)`→ImportError 返回 warning 态；其余异常 error 态（不上抛——结果进任务状态） |

`indexing_contracts.py`：UploadFileInfo/IndexingResult/ReindexResult
TypedDict + `ChunkIndexer` Protocol（index/reindex 固定签名，owner_id 入参）。

### K.9 `application/document_service.py`

| 方法 | 要点 |
|---|---|
| `list_user_documents / get_user_document` | repo 透传+to_summary |
| `delete_document(user_id, doc_id)` | **先 MySQL 删行（权威归属）**→Milvus 软删 chunk（失败 log_degradation 不回滚——残留由 is_deleted 过滤兜底）；归属不符 ResourceNotFoundError |
| `prepare_create(*, user_id, title, …, doc_id=None, last_task_id="")` | 分配/校验 doc_id→写 pending 行 |
| `prepare_replace(*, user_id, doc_id, …, update_title=True)` | `get_owned` 归属校验（无→ValueError→API 400）；**双方 hash 非空且相等→unchanged=true**（幂等跳过 reindex）；否则 mark indexing |
| `bind_task_id(doc_id, task_id)` | 上传提交后回填 |
| `apply_indexing_result(*, doc_id, indexing_result, task_id="")` | 成功→ready+version/chunks；失败→failed+error 摘要 |

`document_indexing_job.py`：`run_document_indexing_job(file_info, *,
task_id="")`（process_file→回写 MySQL；无 doc_id 仅告警）与
`run_document_indexing_job_with_task`（事件通道入口）。
