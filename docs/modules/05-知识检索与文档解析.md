...
| `platform/event_inbox.py` | MySQL Inbox：按 `(event_type,event_id)` 认领、租约、完成与失败审计 |
...
| 📌 `insert_chunks(chunks, *, version=1, content_hash="", owner_id="global", idempotency_key="") -> int` | | 批量 embed→组 rows（含 owner_id 截 64）；有 idempotency_key 用 Milvus upsert，否则 insert；空入参 0 |
| `reindex_document(doc_id, chunks, *, content_hash="", owner_id="global", idempotency_key="")` | | 有 ID 时先查 `evt_{id}_*` 活跃 chunk，命中即复用版本；否则软删旧版→next_version→强制对齐 doc_id→按 ID upsert |
...
| `index(chunks, *, version=1, content_hash="", owner_id="global", idempotency_key="") -> int` | 透传 insert_chunks；事件路径传 task_id |
| `reindex(doc_id, chunks, *, content_hash="", owner_id="global", idempotency_key="")` | 透传 reindex_document（replace 策略 2）；同 task 重放复用已写版本 |
