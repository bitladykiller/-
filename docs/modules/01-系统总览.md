...
│   │   ├── kg/                   # neo4j/t2c/predefined/validation/tenant_cypher
...
| 文件 | 用处 |
|---|---|
| `__init__.py` | 聚合五个子路由 auth/conversations/upload/documents/langgraph |
...
| `infrastructure/kg/tenant_cypher.py` | 租户约束确定性注入：Cypher 执行闸口给每个 MATCH 节点补 `tenant_id` 条件（参数占位 `$__tenant_id`） |
...
| turn_completed / document_index_requested | 两类核心事件；Streams 至少一次投递，Inbox 按 `(tenant_id,event_type,event_id)` 认领；完成后才 ACK | 00 §24 |
...
| 图谱实体关系 | Neo4j | — |
| 上传任务状态 | Redis `tenant:{tenant_id}:task:doc_parse:*`（旧 `task:doc_parse:*` 仅空租户回退） | — |
...
> 行号对齐 v3.37。图例：📌 热路径（每请求经过）｜🔒 有并发语义｜⚠️ 有历史踩坑。
...
contextvars 承载 request_id、已认证 user_id、tenant_id 与 role；`create_task` 自动快照继承。
...
| `set_current_tenant_id` | `(tenant_id: str) -> None` | 写入请求租户边界；空值回落 `default` |
| `get_current_tenant_id` | `() -> str` | 检索/记忆的 SaaS 隔离维度；未设置返回 `default` |
| `set_current_role` | `(role: str) -> None` | 写入请求在租户内的角色 |
| `get_current_role` | `() -> str` | 无认证上下文返回空串 |
| `set_tenant_context` | `(ctx: TenantContext) -> None` | 一次写入 tenant_id/user_id/role 三元组 |
| `get_tenant_context` | `() -> TenantContext \| None` | 未认证上下文返回 None |
| 📌 `TenantContext` | `(tenant_id, user_id, role)` | 请求生命周期内唯一可信租户安全上下文；`get_current_user` 经 JWT + membership 校验通过后调用 `set_tenant_context` 建立 |

WHY 不显式传参：request_id 要出现在**每条**日志、user/tenant 作用域要下沉到检索层，
途经中间件→6 节点→检索器→存储；改所有签名的侵入性远大于请求局部变量。
⚠️ 跨进程（事件 worker）ContextVar 无法传播，必须由事件 payload 显式携带
tenant_id，再在消费者里恢复（见 A.14）。
...
### A.5 `shared/core/rate_limit.py` — SSE 并发限流（租户级 + 用户级双层）

- `class ConcurrencyLimitExceededError(Exception)`：携带 `.limit`
- `class SseConcurrencyLimiter.__init__(redis_client, *, max_concurrent, slot_ttl_seconds, max_concurrent_per_tenant)`；
  双层 key：`ratelimit:sse:{tenant_id}`（租户级配额，`max_concurrent_per_tenant=0` 表示不启用）
  与 `ratelimit:sse:{tenant_id}:user:{user_id}`（用户级护栏，默认 3）

#### 🔒 `async def acquire(self, tenant_id: str, user_id: int) -> None`

先占租户槽位、再占用户槽位，任一超限则 DECR 回滚已占槽位并 raise
（用户级失败时回收刚占的租户槽位，保持两层计数一致）。
单槽逻辑 `INCR → EXPIRE(slot_ttl) → 超限则 DECR 回滚`。
...
两层各 DECR；结果 <0（TTL 清零后多余 release）则 `SET 0` 钳制；失败静默 debug。
...
`(tenant_id,event_type,event_id)` 认领，已完成事件直接 ACK，因此业务效果不会重复。
...
| 📌 `EventInbox.claim(...)` | 唯一键 `(tenant_id,event_type,event_id)`（UNIQUE(tenant_id,event_type,event_id)，租户是审计/清理的一级维度）插入或读取记录；返回 `PROCESS` / `SKIP_COMPLETED` / `BUSY` / `PAYLOAD_CONFLICT` |
...
- `_persist_turn_history(tenant_id, session_id, user_message, assistant_message, *, turn_event_id=None)`：session 非纯数字
