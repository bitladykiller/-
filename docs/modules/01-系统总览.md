...
claim_idle 60s、max_deliveries 3、batch 16。
...
| `_handle_entry(handlers, id, fields, *, deliveries)` | 状态机：畸形→ACK 丢弃；无 handler→ACK 跳过（前向兼容）；成功→ACK；失败且 deliveries≥3→死信+ACK；否则留 PEL 等认领 |
...
失败重放到成功、毒消息进死信、未知类型跳过、ensure_group 幂等。
...
- `AppContainer.build(config)`：task_manager（含孤儿 reconcile）→
  事件基础设施（一个 decode_responses=False 客户端服务 queue+limiter）→
...
- 📌 `handle_turn_completed(payload)`：载荷不完整→warning 丢弃；
  ① `_persist_turn_history`（独立降级）② `after_agent`（记忆链）。
  只有②抛出触发 stream 重试——"单环失败只缺对应视图"
- `_persist_turn_history(session_id, u, a)`：session 非纯数字（历史
  uuid 线程）跳过；MessageRepository.add_turn 一次提交两行
- `handle_document_index_requested(payload)`：复用
  run_task_with_status_updates(origin="stream")——轮询接口对执行通道
  无感知；task_manager 缺失 raise 交给重试
